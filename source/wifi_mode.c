...
{//$$${GpSta:[00,00,01,01,FFFFFF,03,0A,00,1E;00,01,00,FFFFFF,03,0A,01,FF,17:3B,FF]}\r\n
 ...
 //》代表响铃时间（char类型1个字节）00-FF (单位:分钟)
	...
	hex_to_char(&Uart0_TransmitBuffer[42],00);//闹钟ID号默认为0
...
void Wifi_CommandDeal(char *Uart0_Receive)
{
	...
	if(char_compare(WIFI_CMD,"{AlarmSet")==0)//ok
	...
	else if(char_compare(WIFI_CMD,"{AlarmOnOff")==0)
	{//$$${AlarmOnOff:[00,00,10/21/33/12/01/1D/01]}\r\n
	 ...
	 //(WIFI_CMD[11]-'0')  //Programma的ID号暂时没有，为以后扩展预备   
		...
		else if(char_compare(WIFI_CMD, "Turn on unit")==0) 
		{//$$$Turn on unit\r\n
